...
class OnsController extends Controller
{
    ...
    public function pmoCdre()
    {
        ...
        if ($zip->open(storage_path('app/public/pmo '. $mes_zip . ' ' . $ano .'.zip')) === TRUE) {
            $zip->extractTo(storage_path('app/ons/pmo/' . $date . '/'));
        }

        $diretorio = storage_path('app/ons/pmo/'.$date.'/');
        mkdir($diretorio);

        $pathCronograma = storage_path('app/ons/pmo/'. $date. '/'.
            $this->utilOns->encontra_arquivo($diretorio, 'Cronograma'));
